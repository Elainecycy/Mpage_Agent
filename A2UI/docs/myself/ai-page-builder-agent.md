...
| 模型能力 | 纯文本/结构化输出场景，Gemini / GPT-4o 等足够 |
...
| 子能力 | 可行性 | 说明 |
|--------|--------|------|
| 区块检测（header/banner/list/footer） | 高 | 多模态模型（Gemini 2.5 Flash/Pro Vision）可识别常见布局 |
...
│  Mapper 还原树 + 解析 path → 展开为编辑器 component[]      │
...
- **不加 `version` 字段** — 页面 JSON 是业务数据，版本演进由 Schema 文件和 Mapper 代码管理，不在每份页面数据里携带协议版本号。
...
| 定位 | 绝对定位（`position` / `size`） | 流式排版（卡片堆叠） |
| 语义 | 头图上的装饰小图标 | 一张「图标 + 标题/说明」的内容卡片 |

**`BackgroundImage` 的尺寸约定：** 背景图**宽度固定为设计稿宽（375px）**、水平铺满其所在的 Header / Chapter 容器；高度可选 `height`（px），不填时由 Mapper 取图片原始比例换算或使用容器默认高度。**该高度即决定头图区域 / 章节区域的高度**，从而决定其内部 Text、Icon、卡片的可用画布与纵向排布范围。
...
| 属性 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `position` | `{ x, y }` | 是 | 相对**父级背景图**左上角坐标（px，设计稿宽 375） |
| `size` | `{ width, height }` | 是 | 组件占位宽高（px） |
...
> `Text` 和 `Icon` 需要模型（或 Vision）提供 **position / size**。卡片类组件在 MVP 中可继续由 Mapper 按章节背景图内的流式排版自动计算；如果后续需要更高还原度，也可以给卡片组件增加 `position` / `size`，参照系同样是章节背景图。
...
> 用户上传的素材池（含尚未用到的图片）由上传接口返回的 `assetManifest` 在服务端维护（见 §4.2），页面 JSON 的 `data.images` 只保留**实际用到**的图片。
...
| 章节 Chapter | 有章节背景图 → 由其 `height` 决定；无背景图 → 由内部卡片内容撑开 |

#### 3.5.3 定位方式：绝对 vs 流式

| 组件 | 定位方式 | 说明 |
|------|----------|------|
| `Text` / `Icon` | **绝对定位** | 用 JSON 的 `position` / `size`，相对父级背景图左上角 |
| `BackgroundImage` | 铺满 | 宽度撑满父容器，高度见 `height` |
...
```
┌──────────────┐     ┌──────────────────┐     ┌─────────────────┐
│   前端平台    │────▶│  Page Builder    │────▶│  LLM (Vision)   │
│ 上传/对话/预览 │◀────│  Agent Service   │◀────│  + Tools        │
└──────────────┘     └──────────────────┘     └─────────────────┘
                              │
                    ┌─────────┼─────────┐
                    ▼         ▼         ▼
              素材存储   会话状态   Schema 校验
              (OSS)    (页面JSON)  (jsonschema)
```
...
def validate_page_json(json_str: str) -> str:
    """校验结构 + 数据绑定（path 是否能在 data.texts/data.images 解析、images 是否含 url 等），返回错误供模型自我修复"""
```
...
7. 调用 validate_page_json 校验，失败则修正后重试
8. 只输出 JSON，不要 Markdown 包裹，不要加 version 字段
...
- Text 和 Icon 必须包含 position 和 size：Header 内相对头图背景图，Chapter 内相对章节背景图（设计稿宽 375px）
...
| 修改类型 | 策略 | Patch 目标 | 示例 |
|----------|------|-----------|------|
| 改文案 | 改 `data.texts` | `/data/texts/<key>` | 「主标题改成新春特惠」 |
| 换图片 | 改 `data.images` | `/data/images/<key>` | 「头图换成 bg2.png」 |
| 改样式/位置 | 改组件属性 | `/components[i]/style`、`/position` | 「主标题字号调大」 |
| 卡片类型变更 | 替换对应 id 的组件对象 + 增删 data key | `/components[i]` | 「把 card-text-1 改成 BackgroundTextCard」 |
| 结构变更 | 增删组件 + 更新 children + 增删 data key | `/components`、`/children` | 「加一个 Chapter」「删掉 chapter-2」 |

```
/data/texts/mainTitle            ← 改主标题文案（最常见，结构不动）
/data/images/headerBg/url        ← 换头图素材（结构不动）
/components[{index of main-title}]/style/fontSize   ← 改主标题字号
/components[{index of main-title}]/position         ← 改主标题位置
```

- 改文案/换图：优先 patch `/data`，组件树保持不变，最稳。
- 结构变更：增删组件时同步更新父节点 `children`，并**清理或补充** `data.texts` / `data.images` 中对应的 key（删组件要删其引用的 key，避免孤儿数据；加组件要补 key）。
...
def generate_with_validation(llm_output: str, schema: dict, max_retries: int = 2):
    for attempt in range(max_retries + 1):
        try:
            data = parse_json(llm_output)  # 剥离 markdown fence
            jsonschema.validate(data, schema)
            errors = check_integrity(data)   # 第二层校验
            if errors:
...
```python
def check_integrity(page: dict) -> list[str]:
    errors = []
    ...
    for k in set(texts) - used_text_keys:
        errors.append(f"孤儿文案 key（无组件引用）: /texts/{k}")
    for k in set(images) - used_image_keys:
        errors.append(f"孤儿图片 key（无组件引用）: /images/{k}")

    # 7.（可选）children 不应形成环 —— 从 root 做 DFS 检测
    return errors
```
...
```
1. 上传接口
   POST /api/pages/{id}/assets        → 返回 assetManifest

2. 生成接口
   POST /api/pages/{id}/generate
   Body: { assetIds[], userPrompt, mockupUrl? }   # mockupUrl 为 Phase 2（Vision）才启用
...
**`Text` 和 `Icon`** 使用 JSON 中的 `position` / `size` 精确定位：Header 内相对头图背景图，Chapter 内相对章节背景图。卡片类组件在 MVP 中由 Mapper 在章节背景图内流式排版；后续如卡片也需要精确还原，可扩展 `position` / `size`，参照系同样是章节背景图。
...
- [ ] JSON Schema 校验 + 引用/绑定完整性校验 + 1 次自动修复
...
| 坐标偏差大 | 布局凌乱 | Text/Icon 用绝对坐标 + 卡片走流式排版；编辑器内微调 |
...
| 模块 | 推荐 | 理由 |
|------|------|------|
| Agent 框架 | Google ADK / 自研轻量 Orchestrator | ADK 与 A2UI SDK 已有集成样例 |
| 模型 | Gemini 2.5 Flash（生成）+ Pro（复杂 Vision） | 多模态 + 结构化输出成熟 |
| 校验 | `jsonschema`（Python）/ Ajv（TS） | 与 A2UI 一致 |
| 会话存储 | Redis + DB（页面版本） | 支持回滚 |
| 素材 | OSS + manifest 元数据 | 模型只引用 URL/ID，不读二进制 |
...
```json
{
  "components": [
    ...
    { "id": "chapter-1", "component": "Chapter",
      "children": ["chapter-bg-1", "chapter-title-1", "card-bg-text-1", "card-text-1"] },
    { "id": "chapter-bg-1", "component": "BackgroundImage", "src": { "path": "/images/chapterBg1" }, "height": 360 },
    { "id": "chapter-title-1", "component": "Text", "content": { "path": "/texts/chapter1Title" },
      "style": { "fontSize": 20, "color": "#333333", "fontWeight": "bold" },
      "position": { "x": 16, "y": 16 }, "size": { "width": 343, "height": 28 } },
    { "id": "card-bg-text-1", "component": "BackgroundTextCard",
...
此示例可作为 Agent Prompt 中的唯一 few-shot：模型从中学习如何用 `id` + `children` 拼装 Header / Chapter 子树、文案/图片如何写 `path` 绑定到 `data.texts` / `data.images`、Text / Icon 的 position/size、以及何时选用 `TextCard` / `IconCard` / `BackgroundTextCard`。`chapter-2` 无标题，children 仅含卡片 id。
