...
对外生成接口（F5：HTTP 端点 + 并发锁 + 落库）、全量调用日志（脱敏落盘）均已就绪并有单测；
并建好 24 题测试题库,**联真模型(qwen-plus)实测合格率/卡片选对/素材命中三项 100%**（见
`eval/README.md`）。**服务端无遗留欠账**，剩余只有前端线与前后端联调。前端可直接调用。
...
app/
  ...
  services/            生成链路服务
    ...
    call_log.py          全量调用日志：prompt/原始输出/校验错误/token 按 traceId 脱敏落盘
  utils/               预留模块（后续填充）
...
- `tests/unit/test_call_log.py`：调用日志脱敏/序列化/落盘 + 生成成功与超时均落日志（全量调用日志）。
...
- **同步实现**：单次生成是「调用→校验→可能重试」的串行流程，网关用 `httpx.Client` 同步调用；
  ...
  单测注入假网关、不触真实网络。
- **全量调用日志**（`app.services.call_log`）：每次生成把**完整 prompt、模型原始输出、各轮校验
  错误、重试次数、延迟、token** 按 traceId 脱敏后追加到 `MPAGE_CALL_LOG_DIR`（缺省 `./logs`）下
  按天的 JSONL，可按 traceId 还原任意一次请求；脱敏抹掉 `sk-...` 与配置里的真实密钥，写失败只
  告警不影响生成。可经 `MPAGE_CALL_LOG_ENABLED=false` 关闭。
