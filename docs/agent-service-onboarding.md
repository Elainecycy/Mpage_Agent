...
| # | 未完成 | 说明 |
|---|--------|------|
| 1 | **全量调用日志落盘 + 脱敏** | 目前只 log 元数据(traceId/延迟/token),prompt 与模型原始输出**未持久化、未脱敏**(技改 F2.2-3 / F3.1) |
| 2 | **整体联调验收(1.12)** | "后端→真模型→合法 pageJson"已通过题库验证;但"前端 Mapper 渲染 / 编辑器导入"那段**需要前端**,绕不开 |
